package com.example.googlemaps;

public class GeneralData {
	
	private static final String SERVER="http://heroican2.azurewebsites.net/HeroiCan/";
	
	public static final String SERVERCONTROLLER=SERVER+"index.php/site/";
	public static final String SERVER_IMAGES=SERVER+"images/";
	
	//PREFERENCES
	public static final String PREFERENCES_NAME="CafePreferences";
	public static final String PREF_CEDULA="pref_cedula";
	
	//ROLES
	public static final int ROL_CAFICULTOR=0, ROL_PROVEEDOR=1, ROL_COMPRADOR=2;

}